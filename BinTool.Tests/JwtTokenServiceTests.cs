...
public class JwtTokenServiceTests
{
    ...
    {
        var (token, _) = CreateService().CreateToken(User, new[] { AppRoles.Admin });
    ...
    public void Every_role_becomes_its_own_claim()
    {
        var (token, _) = CreateService()
            .CreateToken(User, new[] { AppRoles.Admin, AppRoles.Viewer });
    ...
    public void The_expiry_follows_the_configured_lifetime()
    {
        ...
        var (_, expiresAt) = CreateService().CreateToken(User, Array.Empty<string>());
    ...
    {
        var (token, _) = CreateService().CreateToken(User, Array.Empty<string>());
    ...
    {
        var (token, _) = CreateService().CreateToken(User, new[] { AppRoles.Viewer });
